...
for node,info in land_all_points.items():
    ...
    if counter !=0:
        ...
        new_places_ = DT.translocation(cur_list,center[0],center[1], 0)
...
plt.scatter(landmarks[:, 1], landmarks[:, 0], s=5,c='green',label=f'landmarks {len(landmarks)}')
plt.scatter(landmarks_2[:, 1], landmarks_2[:, 0], s=5,c='blue',label=f'correct')
