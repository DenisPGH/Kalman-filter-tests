...
if __name__ == "__main__":
    ...
    y=np.array([0.3,0.4]).reshape(-1,1)

    regressor.fit(X, y)
    #regressor.fit(np.array(all_points[:,0]).reshape(-1,1), np.array(all_points[:,1]).reshape(-1,1))
    ...
    line = np.linspace(-1, 1, num=10).reshape(-1, 1)
    print(line)
